...
int main(void){
	...
	\item Answers
	\begin{enumerate}
		\item To enable openmp on our local machine (MacBook), we compile the above program with \texttt{gcc-9 -fopenmp hello.c -o hello.out}.
		\item We execute the program on our local machine with \texttt{./hello.out}
